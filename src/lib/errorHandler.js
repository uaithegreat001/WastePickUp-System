 ...
 * Provides user-friendly error messages and tech details for debugging.
 */
...
export const getErrorType = (error) => {
  ...
  if (error.code === 'permission-denied' || error.code === 'unauthenticated') return ErrorTypes.AUTH;
  if (error.code?.startsWith('invalid-') || error.message?.includes('validation')) return ErrorTypes.VALIDATION;
  if (error.name === 'FirebaseError') return ErrorTypes.FIRESTORE;
  if (error.name === 'DOMException' && error.message?.includes('IndexedDB')) return ErrorTypes.INDEXEDDB;
...
export const handleError = (error, context = '', showToast = true, isTechUser = false) => {
